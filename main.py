...
class Function:
    ...
    def plot_function(self,show=True):
        ...
        self.ax.plot_surface(self.X[0], self.X[1], self.value, rstride = 1, cstride = 1, norm = LogNorm(), cmap = cm.jet, linewidth=0, edgecolor='none', alpha=0.5)
        plt.xlabel("x")
        plt.ylabel("y")
...
class PSO:
    ...
    def animate(self):
        ...
        dots = list()
        for i in range(self.num_of_agents):
            d = plt.plot(datasets[i][0], datasets[i][1], markersize=10, markeredgecolor="black", markerfacecolor="purple", marker="o",alpha=1.0)[0]
            dots.append(d)

        anim = animation.FuncAnimation(self.fnc.fig, self.animation_function, frames=self.max_iter, fargs=(datasets,dots), interval=1000, blit=False)
